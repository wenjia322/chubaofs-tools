...
func StartGather(configPath, master string) {
	parseConfig(configPath, master)
...
func (j *Job) appendFile(w *Worker, meta map[uint64]uint64, local, remote *FileInfo) error {
	...
	if local.Size != remote.Size {
		...
		for {
			respData, err := SendByteReq("POST", w.addr+PathReadFile, &RequestGetContent{
				Dir:     j.src,
				Name:    remote.Name,
				Pattern: j.pattern,
				Inode:   remote.Inode,
...
func (w *Worker) remoteFiles(dir, pattern string) ([]*FileInfo, error) {
	...
	respData, err := SendByteReq("POST", w.addr+PathListFile, &req)
	if err != nil {
		return nil, err
	}
...
func remoteDirs(addr, dir, exclusion string) ([]string, error) {
	...
	respData, err := SendByteReq("POST", addr+PathListDir, &req)
	if err != nil {
		return nil, err
	}
